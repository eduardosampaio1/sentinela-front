...
export function formatPercent(value?: number, decimals = 2, _language: UiLanguage = "en") {
...
export function formatMoney(value?: number, _language: UiLanguage = "en") {
  if (value === undefined || value === null || Number.isNaN(value)) {
    return "Data not available";
  }
...
export function formatDate(value?: string, _language: UiLanguage = "en") {
...
export function getMetricNarrative(
  ...
  if (value === undefined || value === null) {
    return {
      status: "Unavailable",
      detail: "This metric is not available in the current run.",
    };
  }
  ...
  if (metric === "similarity") {
    if (value <= 40) {
      return {
        status: "Good",
        detail: "Different intents are staying separated. Your assistant is not collapsing contexts too often.",
      };
    }
    if (value <= 60) {
      return {
        status: "Attention",
        detail: "Some intents are starting to sound alike. Review prompt boundaries before reuse grows.",
      };
    }
    return {
      status: "Bad",
      detail: "Different intents are producing very similar responses. This usually means generic templates are overpowering intent logic.",
    };
  }
  ...
  if (metric === "coverage") {
    if (value >= 85) {
      return {
        status: "Good",
        detail: "Your current dataset covers most expected intents with enough samples to trust the analysis.",
      };
    }
    if (value >= 60) {
      return {
        status: "Attention",
        detail: "Coverage is acceptable, but some intents still need more examples to avoid blind spots.",
      };
    }
    return {
      status: "Bad",
      detail: "Coverage is weak. The model may look stable only because parts of the intent space are under-sampled.",
    };
  }
  ...
  if (value >= 75) {
    return {
      status: "Good",
      detail: "This is in a healthy range and should support more predictable behavior.",
    };
  }
  if (value >= 60) {
    return {
      status: "Attention",
      detail: "This is usable, but still loose enough to create inconsistent user experiences.",
    };
  }
  return {
    status: "Bad",
    detail: "This is below the recommended range and should be treated as a priority problem.",
  };
}

export function getExecutiveSummary(result: AnalysisResult, _language: UiLanguage = "en") {
