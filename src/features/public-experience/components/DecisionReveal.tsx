...
export function DecisionReveal({ result }: { result: ExperienceResult }) {
  ...
  const stages = copy.decision.stages;
  ...
  const status = useMemo(() => copy.decision.status[playbackState as keyof typeof copy.decision.status] ?? copy.decision.status.complete, [copy, playbackState]);
    ...
    >
      <header className="ws-decision-reveal__header">
        <div>
          ...
          <h2 id="ws-decision-reveal-title">{copy.decision.title}</h2>
        </div>
      ...
      <div className="ws-decision-reveal__body">
        <div className="ws-decision-reveal__field" data-stage={activeStage}>
          ...
          <div className="ws-decision-reveal__stages" aria-label={`${copy.decision.replay}: ${stages[activeStage].toLowerCase()}`}>
