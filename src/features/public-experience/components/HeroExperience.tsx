...
export function HeroExperience() {
  const { copy, locale } = usePublicExperience();
  const heroRef = useRef<HTMLElement>(null);
  const [prompt, setPrompt] = useState("");
  const reduceMotion = useReducedMotion();
  const { scrollYProgress } = useScroll({ target: heroRef, offset: ["start start", "end start"] });
  const contentY = useTransform(scrollYProgress, [0, 1], [0, -38]);
  const experience = useExperienceState(locale);
    ...
    <section ref={heroRef} className="ws-hero" data-state={experience.phase} aria-labelledby="ws-hero-title">
      <motion.div className="ws-hero__content" style={reduceMotion ? undefined : { y: contentY }}>
        <div className="ws-hero__content-depth">
          <h1 id="ws-hero-title">{copy.headline}</h1>
          <p className="ws-hero__invitation">{copy.invitation}</p>
          <PromptComposer
            value={prompt}
            onChange={setPrompt}
            onSubmit={submit}
