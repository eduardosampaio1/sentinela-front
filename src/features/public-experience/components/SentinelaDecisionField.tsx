...
export const SentinelaDecisionField = memo(function SentinelaDecisionField({ state, decision }: {
  ...
  const readings = [
    copy.decision.purposeRead,
    decision?.risk ? decision.risk.toUpperCase() : copy.decision.checked,
    compactValue(decision?.contextStrategy ?? copy.decision.contextFit),
    decision?.llmRequired === false ? copy.decision.modelBypassed : copy.decision.routeMatched,
  ];
  const route = decision?.risk === "high"
    ? copy.decision.humanReview
    : decision?.llmRequired === false
      ? copy.decision.safeBypass
      : compactValue(decision?.route ?? copy.decision.controlledOutput);
  ...
  return (
    ...
    >
      ...
      <div className="ws-decision-field__scene" aria-hidden="true">
        <div className="ws-decision-field__edge ws-decision-field__edge--input"><span>{copy.decision.request}</span><i /></div>
        ...
        <div className="ws-decision-field__checks">
          {checks.map((check, index) => (
            <div className="ws-decision-field__check" key={check.key} style={{ "--ws-check-index": index } as CSSProperties}>
              <strong>{copy.decision.checks[index]}</strong><span>{readings[index]}</span>
            </div>
          ))}
        </div>

        <div className="ws-decision-field__decision"><small>{copy.decision.releasedRoute}</small><strong>{route}</strong></div>
        <div className="ws-decision-field__edge ws-decision-field__edge--output"><i /><span>{copy.decision.output}</span></div>
