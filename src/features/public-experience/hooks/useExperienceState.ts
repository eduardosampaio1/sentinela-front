...
export function useExperienceState(locale: PublicExperienceLocale = "en") {
  const [state, dispatch] = useReducer(experienceReducer, initialExperienceState);
  const provider = useMemo(() => createExperienceProvider(locale), [locale]);
  ...
  const submit = useCallback(async (input: string) => {
    ...
    try {
      ...
      if (!request.signal.aborted) {
        dispatch({ type: "ERROR", message: locale === "pt-BR" ? "O Sentinela não conseguiu concluir esta análise. Tente novamente." : "Sentinela could not complete this pass. Try again." });
      }
    } finally {
