...
export function DriftPanel() {
  ...
  const driftConfig = {
    ...
    unknown: { text: "text-[#94A3B8]", label: "Drift data not available for this run", bg: "bg-[rgba(255,255,255,0.02)] border-[rgba(255,255,255,0.06)]" },
  }[driftLevel];
        ...
        <>
          ...
          {data.signals.length > 0 && (
            <div className="space-y-2 border-t border-[rgba(255,255,255,0.05)] pt-3">
              <p className="text-[10px] uppercase tracking-wide font-semibold text-[#475569] mb-2">Signal breakdown</p>
              {data.signals.map((signal) => {
                ...
                return (
                  <div key={signal.key} className="flex items-center gap-3">
                    ...
                    <p className="text-xs text-[#94A3B8] w-12 text-right flex-shrink-0">
